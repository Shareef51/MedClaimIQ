...
from app.domain.regulatory_examination_reclosed_reauthorized_enterprise_remediation_reexecution_surveillance import reclosed_reauthorized_enterprise_remediation_reexecution_surveillance_contract
from app.schemas.regulatory_examination_reclosed_reauthorized_enterprise_remediation_reexecution_surveillance import *
from app.services.regulatory_examination_reclosed_reauthorized_enterprise_remediation_reexecution_surveillance import RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationReExecutionSurveillanceService

router = APIRouter(tags=["regulatory-examination-reclosed-reauthorized-enterprise-remediation-reexecution-surveillance"])
BASE = "/regulatory-examination-reclosed-reauthorized-enterprise-remediation-reexecution-surveillance"
...
def _svc(db, identity):
    return RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationReExecutionSurveillanceService(db, identity.principal.tenant_id)
...
def model():
    return reclosed_reauthorized_enterprise_remediation_reexecution_surveillance_contract()

@router.post(BASE + "/surveillance-snapshots")
def surveillance_snapshot(payload: EnterpriseSurveillanceSnapshotCreate, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).create_surveillance_snapshot(payload.model_dump()))
...
@router.post(BASE + "/root-cause-treatment-decay")
def treatment_decay(payload: RootCauseTreatmentDecayRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).root_cause_treatment_decay(payload.model_dump()))
...
def regression(payload: SystemicControlRetransformationRegressionRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).control_regression(payload.model_dump()))
...
def rebound(payload: EnterpriseRiskReboundRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).rebound(payload.model_dump()))
...
def recurrence(payload: EnterpriseCrossEntityRecurrenceRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).recurrence(payload.model_dump()))
...
def compare(payload: PriorEnterpriseReclosureComparisonRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).compare(payload.model_dump()))
...
def findings(payload: EnterpriseExaminationFindingCorrelationRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).correlate_findings(payload.model_dump()))
...
def followups(payload: EnterpriseRegulatorFollowupLinkageRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).regulator_followups(payload.model_dump()))
...
def materiality(payload: EnterpriseMaterialityRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).materiality(payload.model_dump()))
...
def readiness(payload: EnterpriseReopeningReadinessRequest, request: Request, db: Session = Depends(get_db)):
    identity = _identity(request)
    return _call(lambda: _svc(db, identity).readiness(payload.model_dump()))
