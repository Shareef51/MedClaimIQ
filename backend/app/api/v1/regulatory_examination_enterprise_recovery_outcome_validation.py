...
from app.domain.regulatory_examination_enterprise_recovery_outcome_validation import enterprise_recovery_outcome_contract
from app.schemas.regulatory_examination_enterprise_recovery_outcome_validation import *
...
router = APIRouter(prefix="/regulatory-examination-enterprise-recovery-outcome-validation", tags=["regulatory-examination-enterprise-recovery-outcome-validation"])

def svc(x_tenant_id: str = Header(default="default")):
    return RegulatoryExaminationEnterpriseRecoveryOutcomeValidationService(None, x_tenant_id)

@router.get("/model")
def model(): return enterprise_recovery_outcome_contract()
