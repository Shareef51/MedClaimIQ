...
class EnterpriseRemediationProgramCreate(BaseModel):
    actor_role: str
    remediation_program_id: str
    enterprise_remediation_reauthorization_version_id: str
    release99_investigation_version_id: str
    release99_investigation_conclusion_version_id: str
    release99_root_cause_confirmation_version_id: str
    release99_systemic_failure_classification_version_id: str
    release99_strategy_candidate_version_id: str
    release99_independent_challenge_version_id: str
    release99_human_reauthorization_confirmed: bool
    release99_reauthorization_decision: Literal["authorize","reject","defer"]
    evidence_refs: list[str] = Field(min_length=1)
    workstreams: list[dict[str,Any]] = Field(default_factory=list)

class EnterpriseRemediationAnalysisRequest(BaseModel):
...
class ControlRetransformationApprovalCreate(BaseModel):
    actor_role: str; remediation_program_id: str; enterprise_remediation_execution_version_id: str
    control_ids: list[str] = Field(min_length=1)
    release99_reauthorization_scope_references: list[str] = Field(min_length=1)
...
class IndependentRecoveryEffectivenessAssuranceCreate(BaseModel):
    reviewer_role: str; remediation_program_id: str; enterprise_remediation_execution_version_id: str
    implementation_owner_id: str | None = None
    tests: list[dict[str,Any]] = Field(min_length=1)
    evidence_refs: list[str] = Field(min_length=1)

class ExecutionReadinessRequest(BaseModel):
    release99_enterprise_remediation_reauthorization_reference_present: bool=False
    release99_human_reauthorization_confirmed: bool=False
