...
class EnterpriseSurveillanceSnapshotCreate(BaseModel):
    recovery_program_id: str
    release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id: str
    surveillance_type: Literal["scheduled", "event_driven", "regulatory_followup", "control_regression", "risk_rebound", "cross_entity_recurrence"] = "scheduled"
    summary: str
...
class EnterpriseRecoveryDecayRequest(BaseModel):
    recovery_program_id: str
    release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id: str
    release105_reclosure_control_health_score: float = 100.0
...
class EnterpriseRiskReboundRequest(BaseModel):
    recovery_program_id: str
    release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id: str
    release105_reclosure_systemic_risk_score: float
...
class EnterpriseExecutiveInternalAuditChallengeCreate(BaseModel):
    ...
    evidence_refs: list[str] = Field(default_factory=list)
    investigation_owner_id: str | None = None
    independent_reassessor_id: str | None = None

class EnterpriseReopeningReadinessRequest(BaseModel):
    release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_reference_validated: bool = False
...
class EnterpriseSurveillanceDashboardRequest(BaseModel):
    ...
    release105_reclosure_control_health_score: float = 100.0
    current_control_health_score: float = 100.0
    release105_reclosure_systemic_risk_score: float = 0.0
