...
class RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationReExecutionSurveillanceService:
    ...
    def _release105(self, payload: dict):
        if not payload.get("release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id"):
            raise ValueError("Release 105 reauthorized enterprise remediation re-execution sustainability reclosure version is required")

    def create_surveillance_snapshot(self, payload: dict):
        self._release105(payload)
        if not payload.get("evidence_refs"):
            raise ValueError("evidence-bound surveillance snapshot is required")
        return self._immutable({
            "reauthorized_enterprise_remediation_reexecution_surveillance_version_id": str(uuid4()),
    ...
    def create_investigation(self, actor_id: str, payload: dict):
        ...
        return self._immutable({
            "reauthorized_enterprise_remediation_reexecution_decay_investigation_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
    ...
    def independent_reassess(self, actor_id: str, payload: dict):
        ...
        return self._immutable({
            "reauthorized_enterprise_remediation_reexecution_independent_reassessment_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
    ...
    def enterprise_challenge(self, actor_id: str, payload: dict):
        ...
        if payload.get("investigation_owner_id") and str(payload.get("investigation_owner_id")) == str(actor_id):
            raise PermissionError("segregation of duties: investigation owner cannot perform executive/internal-audit challenge")
        if payload.get("independent_reassessor_id") and str(payload.get("independent_reassessor_id")) == str(actor_id):
            raise PermissionError("segregation of duties: independent reassessor cannot challenge their own reassessment")
        if not payload.get("evidence_refs"):
            raise ValueError("evidence-bound executive/internal-audit challenge is required")
        return self._immutable({
            "reauthorized_enterprise_remediation_reexecution_reopening_challenge_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "human_decision": True,
            "automated_reopening": False,
            "decided_by": actor_id,
    ...
    def decide_reopening(self, actor_id: str, payload: dict):
        ...
        return self._immutable({
            "reauthorized_enterprise_remediation_reexecution_reopening_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
