...
from app.evaluation.regulatory_examination_reopened_enterprise_recovery_investigation import (
    ...
    enterprise_recovery_investigation_dashboard,
    audit_export_manifest,
)

INVESTIGATOR_ROLES = {"internal_auditor", "chief_audit_executive", "independent_assurance", "regulatory_remediation_lead", "enterprise_recovery_investigator"}
...
class RegulatoryExaminationReopenedEnterpriseRecoveryInvestigationService:
    ...
    def create_investigation(self, actor_id: str, p: dict):
        ...
        if not p.get("release98_enterprise_reopening_version_id"):
            raise ValueError("Release 98 human enterprise reopening reference required")
        if not p.get("surveillance_version_refs") or not p.get("evidence_refs"):
            raise ValueError("Release 98 surveillance versions and evidence are required")
        return self._immutable({
            "reopened_enterprise_recovery_investigation_version_id": str(uuid4()),
    ...
    def reconstruct_evidence(self, p): return {"tenant_id": self.tenant_id, "analysis_only": True, **reconstruct_multi_cycle_enterprise_evidence(p)}
    def reconstruct_root_causes(self, p): return {"tenant_id": self.tenant_id, "recommendation_only": True, **reconstruct_systemic_recovery_failure_root_causes(p)}
    def validate_assumptions(self, p): return {"tenant_id": self.tenant_id, "analysis_only": True, **validate_prior_enterprise_recertification_reclosure_assumptions(p)}
    ...
    def dashboard(self, p): return {"tenant_id": self.tenant_id, **enterprise_recovery_investigation_dashboard(p)}
    ...
    def confirm_root_causes(self, actor_id: str, p: dict):
        if p.get("actor_role") not in INVESTIGATOR_ROLES:
            raise PermissionError("authorized human enterprise root-cause confirmation required")
        ...
        return self._immutable({
            "enterprise_recovery_root_cause_confirmation_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
    ...
    def confirm_systemic_failure_classification(self, actor_id: str, p: dict):
        if p.get("actor_role") not in CLASSIFICATION_ROLES:
            raise PermissionError("authorized human enterprise systemic-failure classification confirmation required")
        ...
        return self._immutable({
            "enterprise_recovery_systemic_failure_classification_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "human_confirmation": True,
            "automated_confirmation": False,
            "confirmed_by": actor_id,
    ...
    def create_strategy_candidate(self, actor_id: str, p: dict):
        required = ["release98_enterprise_reopening_version_id", "investigation_version_id", "root_cause_confirmation_version_id", "systemic_failure_classification_version_id"]
        if any(not p.get(k) for k in required) or not p.get("evidence_refs"):
            raise ValueError("Release 98 reopening, investigation, human confirmations and evidence are required")
        return self._immutable({
    ...
    def independent_challenge(self, actor_id: str, p: dict):
        ...
        if actor_id == p.get("investigation_owner_actor_id"):
            raise PermissionError("segregation of duties requires a different independent reviewer")
        ...
        return self._immutable({
            "enterprise_recovery_independent_challenge_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "human_decision": True,
    ...
    def conclude_investigation(self, actor_id: str, p: dict):
        ...
        return self._immutable({
            "reopened_enterprise_recovery_investigation_conclusion_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
    ...
    def authorize_enterprise_remediation(self, actor_id: str, p: dict):
        ...
        required = [
            "release98_enterprise_reopening_version_id", "investigation_version_id", "investigation_conclusion_version_id",
