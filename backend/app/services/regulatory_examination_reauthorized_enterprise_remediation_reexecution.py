...
from app.evaluation.regulatory_examination_reauthorized_enterprise_remediation_reexecution import *
...
class RegulatoryExaminationReauthorizedEnterpriseRemediationReExecutionService:
    ...
    def create_program(self,actor_id,p):
        ...
        required=["release103_recovery_program_id","enterprise_remediation_reauthorization_version_id","release103_investigation_version_id","release103_investigation_conclusion_version_id","release103_root_cause_confirmation_version_id","release103_systemic_remediation_failure_classification_version_id","release103_strategy_candidate_version_id","release103_independent_challenge_version_id"]
        if any(not p.get(k) for k in required): raise ValueError("complete Release 103 enterprise remediation reauthorization provenance is required")
        if p.get("release103_human_reauthorization_confirmed") is not True or p.get("release103_reauthorization_decision")!="authorize": raise ValueError("confirmed Release 103 human enterprise remediation authorization is required")
        if not p.get("evidence_refs"): raise ValueError("evidence-bound Release 103 reauthorization provenance is required")
        return self._immutable({"enterprise_remediation_reexecution_version_id":str(uuid4()),"tenant_id":self.tenant_id,"release103_human_enterprise_remediation_reauthorization_reference_required":True,"human_program_intake":True,"automated_program_approval":False,"created_by":actor_id,"created_at":self._now(),**p})
    ...
    def approve_control_retransformation(self,actor_id,p):
        ...
        if not p.get("enterprise_remediation_reexecution_version_id") or not p.get("control_ids"): raise ValueError("enterprise remediation re-execution and control references required")
        if not p.get("release103_reauthorization_scope_references") or not p.get("root_cause_treatment_references") or not p.get("evidence_refs"): raise ValueError("Release 103 scope, root-cause treatment and evidence references required")
    ...
    def create_checkpoint(self,actor_id,p):
        if p.get("actor_role") not in EXECUTION_ROLES: raise PermissionError("authorized human enterprise remediation re-execution role required")
        if not p.get("enterprise_remediation_reexecution_version_id") or not p.get("evidence_refs"): raise ValueError("enterprise remediation re-execution version and evidence are required")
        return self._immutable({"enterprise_remediation_reexecution_checkpoint_version_id":str(uuid4()),"tenant_id":self.tenant_id,"human_checkpoint":True,"automated_completion_certification":False,"recorded_by":actor_id,"recorded_at":self._now(),**p})
    def independent_assurance(self,actor_id,p):
        ...
        if not p.get("enterprise_remediation_reexecution_version_id") or not p.get("tests") or not p.get("evidence_refs"): raise ValueError("execution version, tests and evidence references are required")
    ...
    def executive_review(self,actor_id,p):
        ...
        if not p.get("enterprise_remediation_reexecution_version_id"): raise ValueError("enterprise remediation re-execution version is required")
