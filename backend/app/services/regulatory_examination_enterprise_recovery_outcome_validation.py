...
class RegulatoryExaminationEnterpriseRecoveryOutcomeValidationService:
    ...
    def outcomes(self, p: dict):
        if not p.get("release96_enterprise_recovery_execution_version_id"):
            raise ValueError("Release 96 enterprise recovery execution version is required")
    ...
    def independent_validate(self, actor_id: str, p: dict):
        ...
        if not p.get("release96_enterprise_recovery_execution_version_id") or not p.get("release96_independent_effectiveness_assurance_version_id"):
            raise ValueError("Release 96 enterprise execution and independent effectiveness assurance references are required")
        ...
        return self._immutable({
            "enterprise_independent_outcome_validation_version_id": str(uuid4()),
    ...
    def sustainability(self, p: dict):
        ...
        return self._immutable({
            "enterprise_sustainability_assessment_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "analysis_only": True,
    ...
    def residual_risk_reassessment(self, actor_id: str, p: dict):
        ...
        required = [
            ...
            "release96_independent_effectiveness_assurance_version_id",
            "independent_outcome_validation_version_id",
            "sustainability_assessment_version_id",
        ]
        if any(not p.get(k) for k in required):
            raise ValueError("Release 96 execution/assurance, independent outcome validation and sustainability references are required")
        if not p.get("evidence_refs"):
            raise ValueError("evidence-bound residual-risk reassessment is required")
        return self._immutable({
            "enterprise_residual_risk_decision_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "human_decision": True,
    ...
    def recertify_recovery(self, actor_id: str, p: dict):
        ...
        if any(not p.get(k) for k in required):
            raise ValueError("Release 96 execution/assurance, independent outcome validation, residual-risk and sustainability references are required")
        ...
        return self._immutable({
            "enterprise_recovery_recertification_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "human_decision": True,
    ...
    def reclose_program(self, actor_id: str, p: dict):
        ...
        if not p.get("enterprise_recovery_recertification_version_id"):
            raise ValueError("human executive enterprise recovery recertification reference is required")
        ...
        return self._immutable({
            "enterprise_sustainability_reclosure_version_id": str(uuid4()),
            "tenant_id": self.tenant_id,
            "human_decision": True,
            "automated_reclosure": False,
            "decided_by": actor_id,
