...
def run_reauthorized_enterprise_remediation_outcome_monitor(items: list[dict]) -> dict:
    alerts = []
    for item in items:
        outcomes = enterprise_recovery_outcomes(item)
        ...
        health = cross_entity_control_health(item)
        ...
        if (
            ...
            or (risk["release100_baseline_systemic_risk_score"] and not risk["risk_reduction_target_met"])
            or commitments["unresolved_commitment_ids"]
            or not blockers["unresolved_blockers_cleared"]
            or (health["entity_count"] and not health["cross_entity_control_health_stabilized"])
            ...
            alerts.append({
                ...
                "cross_entity_control_health_stabilized": health["cross_entity_control_health_stabilized"],
