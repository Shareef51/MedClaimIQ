...
def run_reopened_enterprise_recovery_investigation_monitor(items: list[dict]) -> dict:
    alerts=[]
    for item in items:
        program_id=item.get("recovery_program_id")
        evidence=reconstruct_multi_cycle_enterprise_evidence(item.get("evidence_reconstruction", {}))
        roots=reconstruct_systemic_recovery_failure_root_causes(item.get("root_cause_reconstruction", {}))
        ...
        if evidence["repeated_enterprise_recovery_failure_pattern"]:
            alerts.append({"recovery_program_id":program_id,"kind":"repeated_enterprise_recovery_failure","severity":"high"})
