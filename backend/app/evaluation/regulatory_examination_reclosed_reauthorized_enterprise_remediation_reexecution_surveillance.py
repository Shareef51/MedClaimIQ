...
def root_cause_treatment_decay(payload: dict) -> dict:
    treatments = payload.get("root_cause_treatments", payload.get("treatments", []))
    decayed, persistent, emergent, missing = [], [], [], []
    for treatment in treatments:
        tid = str(treatment.get("treatment_id", treatment.get("root_cause_id", "")))
        prior = str(treatment.get("release105_reclosure_status", treatment.get("prior_status", "effective"))).lower()
        current = str(treatment.get("current_status", treatment.get("status", ""))).lower()
        failures = int(treatment.get("post_reclosure_failure_count", treatment.get("failure_count", 0)) or 0)
        decay = bool(treatment.get("decay")) or current in {"degraded", "failed", "ineffective", "recurred"} or (prior in {"effective", "stable", "passed"} and current not in {"effective", "stable", "passed"})
        if decay:
            decayed.append(tid)
            if str(treatment.get("root_cause_type", "")).lower() == "persistent" or bool(treatment.get("persistent_root_cause")):
                persistent.append(tid)
            if str(treatment.get("root_cause_type", "")).lower() == "emergent" or bool(treatment.get("emergent_root_cause")):
                emergent.append(tid)
            if not treatment.get("evidence_refs"):
                missing.append(tid)
    denom = max(1, len(treatments))
    pct = round(100.0 * len(set(decayed)) / denom, 2)
    threshold = float(payload.get("root_cause_treatment_decay_threshold_percent", 20) or 20)
    return {
        "root_cause_treatment_count": len(treatments),
        "decayed_root_cause_treatment_ids": sorted(x for x in set(decayed) if x),
        "persistent_root_cause_treatment_decay_ids": sorted(x for x in set(persistent) if x),
        "emergent_root_cause_treatment_decay_ids": sorted(x for x in set(emergent) if x),
        "missing_evidence_root_cause_treatment_ids": sorted(x for x in set(missing) if x),
        "root_cause_treatment_decay_percent": pct,
        "material_root_cause_treatment_decay_candidate": bool(persistent or emergent) or pct >= threshold,
        "human_validation_required": bool(decayed),
        "automatic_root_cause_determination_allowed": False,
    }


def multi_cycle_enterprise_recovery_decay(payload: dict) -> dict:
    baseline = float(payload.get("release105_reclosure_control_health_score", payload.get("reclosure_control_health_score", 100)) or 100)
    ...
    return {
        ...
        "reexecution_recovery_cycle_count": int(payload.get("reexecution_recovery_cycle_count", repeat_cycles + 1) or repeat_cycles + 1),
        "sustainability_breach_count": breaches,
        "regulatory_commitment_breach_count": commitment_breaches,
        "repeated_reexecution_recovery_failure_candidate": repeat_cycles >= 2 and (regressions > 0 or breaches > 0 or health_decay >= 10 or commitment_breaches > 0),
...
def systemic_control_retransformation_regression(payload: dict) -> dict:
    ...
    for control in controls:
        ...
        prior = str(control.get("release105_reclosure_status", control.get("prior_status", ""))).lower()
...
def systemic_risk_rebound(payload: dict) -> dict:
    baseline = float(payload.get("release105_reclosure_systemic_risk_score", payload.get("reclosure_systemic_risk_score", 0)) or 0)
    ...
    return {
        "release105_reclosure_systemic_risk_score": baseline,
...
def prior_enterprise_reclosure_comparison(payload: dict) -> dict:
    ...
    return {
        "release105_systemic_recovery_recertification_version_id": prior.get("enterprise_recovery_recertification_version_id"),
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": prior.get("enterprise_sustainability_reclosure_version_id"),
...
def enterprise_materiality(payload: dict) -> dict:
    ...
    treatment_decay = float(payload.get("root_cause_treatment_decay_percent", 0) or 0)
    ...
    return {
        ...
        "reexecution_remediation_failure_candidate": score >= 25 and (repeat_cycles >= 1 or treatment_decay > 0 or regression > 0),
...
def enterprise_reopening_readiness(payload: dict) -> dict:
    checks = {
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_reference_validated": bool(payload.get("release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_reference_validated")),
        "material_systemic_recovery_decay_confirmed": bool(payload.get("material_systemic_recovery_decay_confirmed")),
        "root_cause_treatment_decay_scope_human_validated": bool(payload.get("root_cause_treatment_decay_scope_human_validated")),
...
def supervisory_dashboard_summary(payload: dict) -> dict:
    ...
    treatment_decay = root_cause_treatment_decay(payload)
    ...
    return {
        ...
        "decay": decay,
        "root_cause_treatment_decay": treatment_decay,
...
def audit_export_manifest(payload: dict) -> dict:
    ...
    return {
        ...
        "recovery_program_id": payload.get("recovery_program_id"),
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": payload.get("release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id"),
        "evidence_refs": refs,
