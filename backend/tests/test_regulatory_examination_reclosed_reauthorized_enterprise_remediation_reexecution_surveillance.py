from app.domain.regulatory_examination_reclosed_reauthorized_enterprise_remediation_reexecution_surveillance import (
    RECLOSED_REAUTHORIZED_ENTERPRISE_REMEDIATION_REEXECUTION_SURVEILLANCE_AUTHORITY,
    reclosed_reauthorized_enterprise_remediation_reexecution_surveillance_contract,
)
...
from app.services.regulatory_examination_reclosed_reauthorized_enterprise_remediation_reexecution_surveillance import (
    RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationReExecutionSurveillanceService,
)


def test_release106_non_delegable_authority_and_release105_provenance():
    authority = RECLOSED_REAUTHORIZED_ENTERPRISE_REMEDIATION_REEXECUTION_SURVEILLANCE_AUTHORITY
    assert authority["release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_reference_required"]
    ...
    service = RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationReExecutionSurveillanceService(None, "tenant-a")
    try:
        service.decay({"recovery_program_id": "rp1"})
    except ValueError:
        pass
    else:
        raise AssertionError("Release 105 reauthorized enterprise remediation re-execution sustainability reclosure provenance must be mandatory")
    try:
        service.control_regression({"recovery_program_id": "rp1", "controls": []})
    except ValueError:
        pass
    else:
        raise AssertionError("all surveillance analysis paths must preserve Release 105 reclosure provenance")


def test_release106_multi_cycle_decay_systemic_regression_rebound_and_recurrence():
    decay = multi_cycle_enterprise_recovery_decay({
        "release105_reclosure_control_health_score": 96,
    ...
    assert decay["repeated_reexecution_recovery_failure_candidate"]
    assert decay["human_investigation_required"]

    treatment_decay = root_cause_treatment_decay({"root_cause_treatments": [
        {"treatment_id": "rt-persistent", "root_cause_type": "persistent", "release105_reclosure_status": "effective", "current_status": "failed", "evidence_refs": ["rt1"]},
        {"treatment_id": "rt-emergent", "root_cause_type": "emergent", "release105_reclosure_status": "effective", "current_status": "degraded", "evidence_refs": ["rt2"]},
    ]})
    assert treatment_decay["material_root_cause_treatment_decay_candidate"]
    assert treatment_decay["root_cause_treatment_decay_percent"] == 100.0

    regression = systemic_control_retransformation_regression({"controls": [
        {"control_id": "c1", "release105_reclosure_status": "effective", "current_status": "failed", "severity": "critical", "post_reclosure_failure_count": 2, "evidence_refs": ["e1"]},
    ...
    rebound = systemic_risk_rebound({
        "release105_reclosure_systemic_risk_score": 20,
...
def test_release106_prior_reclosure_materiality_and_reopening_readiness():
    ...
    readiness = enterprise_reopening_readiness({
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_reference_validated": True,
        "material_systemic_recovery_decay_confirmed": True,
        "root_cause_treatment_decay_scope_human_validated": True,
...
def test_release106_human_only_investigation_independent_reassessment_and_reopening():
    service = RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationReExecutionSurveillanceService(None, "tenant-a")
    try:
        service.create_investigation("ai", {
            "actor_role": "ai_agent",
            "recovery_program_id": "rp1",
            "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": "sr1",
    ...
    snapshot = service.create_surveillance_snapshot({
        "recovery_program_id": "rp1",
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": "sr1",
        "surveillance_type": "control_regression",
        "summary": "material re-execution control regression detected",
        "evidence_refs": ["e1", "e2"],
        "metrics": {"decay_score": 82},
    })
    assert snapshot["immutable"] and snapshot["monitoring_only"] and not snapshot["automated_reopening"]

    investigation = service.create_investigation("investigator-1", {
        "actor_role": "compliance_reviewer",
        "recovery_program_id": "rp1",
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": "sr1",
        "summary": "material systemic re-execution decay confirmed for investigation",
        "surveillance_version_refs": [snapshot["reauthorized_enterprise_remediation_reexecution_surveillance_version_id"]],
        "enterprise_materiality_version_ref": "mat106-v1",
        "evidence_refs": ["e1", "e2"],
    })
    inv_id = investigation["reauthorized_enterprise_remediation_reexecution_decay_investigation_version_id"]
    reassessment = service.independent_reassess("ia-2", {
        "actor_role": "independent_assurance",
        "recovery_program_id": "rp1",
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": "sr1",
        "result": "confirmed_decay",
        "conclusion": "independent evidence confirms material systemic decay",
        "investigation_version_id": inv_id,
        "investigation_owner_id": "investigator-1",
        "evidence_refs": ["e1", "e2"],
    })
    reassessment_id = reassessment["reauthorized_enterprise_remediation_reexecution_independent_reassessment_version_id"]
    challenge = service.enterprise_challenge("cro-1", {
        "actor_role": "chief_risk_officer",
        "recovery_program_id": "rp1",
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": "sr1",
        "decision": "escalate",
        "investigation_version_id": inv_id,
        "independent_reassessment_version_id": reassessment_id,
        "investigation_owner_id": "investigator-1",
        "independent_reassessor_id": "ia-2",
        "rationale": "reopening criteria require executive decision",
        "evidence_refs": ["e1", "e2"],
    })
    readiness = {
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_reference_validated": True,
    ...
    reopened = service.decide_reopening("cro-2", {
        "actor_role": "executive_certifier",
        "decision": "reopen",
        "rationale": "deterministic reopening gates satisfied",
        "recovery_program_id": "rp1",
        "release105_reauthorized_enterprise_remediation_reexecution_sustainability_reclosure_version_id": "sr1",
        "investigation_version_id": inv_id,
        "independent_reassessment_version_id": reassessment_id,
        "enterprise_challenge_version_id": challenge["reauthorized_enterprise_remediation_reexecution_reopening_challenge_version_id"],
        "readiness": readiness,
    })
    assert reopened["human_reopening"] and not reopened["automated_reopening"]
    assert reopened["immutable"]
    assert "human enterprise reopening" in reclosed_reauthorized_enterprise_remediation_reexecution_surveillance_contract()["traceability"]
