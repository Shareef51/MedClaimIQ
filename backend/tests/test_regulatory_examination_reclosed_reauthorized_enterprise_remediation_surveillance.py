from app.domain.regulatory_examination_reclosed_reauthorized_enterprise_remediation_surveillance import (
    RECLOSED_REAUTHORIZED_ENTERPRISE_REMEDIATION_SURVEILLANCE_AUTHORITY,
    reclosed_reauthorized_enterprise_remediation_surveillance_contract,
)
...
from app.services.regulatory_examination_reclosed_reauthorized_enterprise_remediation_surveillance import (
    RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationSurveillanceService,
)


def test_release102_non_delegable_authority_and_release101_provenance():
    authority = RECLOSED_REAUTHORIZED_ENTERPRISE_REMEDIATION_SURVEILLANCE_AUTHORITY
    assert authority["release101_reauthorized_enterprise_remediation_sustainability_reclosure_reference_required"]
    ...
    service = RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationSurveillanceService(None, "tenant-a")
    try:
        service.decay({"recovery_program_id": "rp1"})
    except ValueError:
        pass
    else:
        raise AssertionError("Release 101 reauthorized enterprise remediation sustainability reclosure provenance must be mandatory")


def test_release102_multi_cycle_decay_systemic_regression_rebound_and_recurrence():
    decay = multi_cycle_enterprise_recovery_decay({
        "release101_reclosure_control_health_score": 96,
    ...
    treatment_decay = root_cause_treatment_decay({"root_cause_treatments": [
        {"treatment_id": "rt-persistent", "root_cause_type": "persistent", "release101_reclosure_status": "effective", "current_status": "failed", "evidence_refs": ["rt1"]},
        {"treatment_id": "rt-emergent", "root_cause_type": "emergent", "release101_reclosure_status": "effective", "current_status": "degraded", "evidence_refs": ["rt2"]},
    ]})
    ...
    regression = systemic_control_retransformation_regression({"controls": [
        {"control_id": "c1", "release101_reclosure_status": "effective", "current_status": "failed", "severity": "critical", "post_reclosure_failure_count": 2, "evidence_refs": ["e1"]},
    ...
    rebound = systemic_risk_rebound({
        "release101_reclosure_systemic_risk_score": 20,
...
def test_release102_prior_reclosure_materiality_and_reopening_readiness():
    ...
    readiness = enterprise_reopening_readiness({
        "release101_reauthorized_enterprise_remediation_sustainability_reclosure_reference_validated": True,
...
def test_release102_human_only_investigation_independent_reassessment_and_reopening():
    service = RegulatoryExaminationReclosedReauthorizedEnterpriseRemediationSurveillanceService(None, "tenant-a")
    try:
        service.create_investigation("ai", {
            "actor_role": "ai_agent",
            "recovery_program_id": "rp1",
            "release101_reauthorized_enterprise_remediation_sustainability_reclosure_version_id": "sr1",
    ...
    assert "human enterprise reopening" in reclosed_reauthorized_enterprise_remediation_surveillance_contract()["traceability"]
