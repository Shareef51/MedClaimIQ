...
export async function GET(request: NextRequest, context: { params: Promise<{ claimId: string }> }) {
  ...
  try {
    const upstream = await backendRequest(`/api/v1/claims/${encodeURIComponent(claimId)}/realtime/events?after_sequence=${encodeURIComponent(after)}`, {
      headers: { Accept: "text/event-stream" },
      signal: request.signal
    });
    if (!upstream.ok || !upstream.body) {
      return NextResponse.json({ error: "upstream_stream_unavailable" }, { status: upstream.status || 502 });
    }
