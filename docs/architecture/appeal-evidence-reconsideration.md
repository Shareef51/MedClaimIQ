# Release 38 — Appeal Evidence Re-Ingestion and Independent Reconsideration
...
2. Release 38 registers a pending appeal re-ingestion record without mutating the original decision.
...
10. The separate Release 36 governed human resolution path remains the only mechanism that can affirm, modify or overturn the controlling claim outcome.
...
Every persisted stage is tenant scoped; Release 38 tables use PostgreSQL RLS. Locked snapshots and AI/RAG/reviewer evidence records are protected against payload rewriting by database triggers.
